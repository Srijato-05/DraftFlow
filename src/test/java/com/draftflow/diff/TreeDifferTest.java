...
public class TreeDifferTest {
    ...
    @Test
    public void testTypeChangeDiff() throws IOException {
        String blobHash = cas.writeObject(new Blob("content".getBytes()));

        // Tree A has a file named "node"
        TreeEntry fileEntry = new TreeEntry("node", blobHash, ObjectType.BLOB, 100644);
        String treeA = cas.writeObject(new Tree(Arrays.asList(fileEntry)));

        // Tree B has a directory named "node" which contains "subfile"
        TreeEntry subEntry = new TreeEntry("subfile", blobHash, ObjectType.BLOB, 100644);
        String subTreeB = cas.writeObject(new Tree(Arrays.asList(subEntry)));
        TreeEntry dirEntry = new TreeEntry("node", subTreeB, ObjectType.TREE, 040000);
        String treeB = cas.writeObject(new Tree(Arrays.asList(dirEntry)));

        // Diff A -> B: File "node" deleted, Directory "node/subfile" added
        List<FileDiff> diffsAB = TreeDiffer.diff(treeA, treeB, cas);
        assertEquals(2, diffsAB.size());
        assertEquals("node", diffsAB.get(0).getPath());
        assertEquals(DiffType.DELETED, diffsAB.get(0).getType());
        assertEquals("node/subfile", diffsAB.get(1).getPath());
        assertEquals(DiffType.ADDED, diffsAB.get(1).getType());

        // Diff B -> A: Directory "node/subfile" deleted, File "node" added
        List<FileDiff> diffsBA = TreeDiffer.diff(treeB, treeA, cas);
        assertEquals(2, diffsBA.size());
        assertEquals("node", diffsBA.get(0).getPath());
        assertEquals(DiffType.ADDED, diffsBA.get(0).getType());
        assertEquals("node/subfile", diffsBA.get(1).getPath());
        assertEquals(DiffType.DELETED, diffsBA.get(1).getType());
    }
}
