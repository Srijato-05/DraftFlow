/**
 * @file SignatureHelper.java
 * @description Cryptographic signature utility for DraftFlow VCS.
 * Implements Elliptic Curve Digital Signature Algorithm (ECDSA with SHA-256) keypair generation, 
 * message signing, and signature verification to enforce revision authenticity and non-repudiation.
 * 
 * DESIGN RATIONALE:
 * - Uses 256-bit ECDSA keys (secp256r1) which provide robust security while yielding much smaller
 *   signature sizes compared to traditional RSA, reducing metadata footprint in the object store.
 * - Signs revision tree nodes using local private keys (`id_ecdsa`) and packages public keys directly
 *   into revision manifests for decentralized client verification.
 */
