/**
 * @file FSWatcher.java
 * @description OS-level file system change notification watcher for DraftFlow VCS.
 * Utilizes the native Java `WatchService` system API to monitor file creations, edits, and deletions
 * inside the repository directory recursively.
 * 
 * DESIGN RATIONALE:
 * - Employs reactive OS-level file system events rather than polling to minimize CPU utilization.
 * - Walk recursively registers folder paths on startup, dynamically registering new subfolders when created on disk.
 * - Implements a 500ms debounce scheduled executor to group rapid consecutive file events (such as bulk edits,
 *   build compiles, or copy actions) into a single batch commit scan, preventing event storm congestion.
 */
