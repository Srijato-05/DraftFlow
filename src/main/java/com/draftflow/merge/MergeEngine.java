/**
 * @file MergeEngine.java
 * @description The core 3-way merge engine for DraftFlow VCS.
 * Computes tree-level merges between branch pointer commits by identifying their Lowest Common Ancestor (LCA),
 * flattening the hierarchical directory structures, and reconciling content modifications on a file-by-file basis.
 * 
 * DESIGN RATIONALE:
 * - Employs a robust 3-way diff algorithm: compares `Ours` vs `Theirs` relative to a shared `Base` ancestor.
 * - Handles tree modifications (additions, deletions, permission changes) cleanly by scanning paths.
 * - If both branches modify the same text file, delegating to the `LineMerge` utility to resolve block alignments;
 *   if conflicts persist, a `ConflictNode` is written to the CAS object store to track conflicting hashes.
 */
