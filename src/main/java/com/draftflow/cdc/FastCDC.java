/**
 * @file FastCDC.java
 * @description High-performance Content-Defined Chunking (FastCDC) engine.
 * Splits large files into variable-sized chunks based on data content boundaries instead of fixed offsets.
 * 
 * DESIGN RATIONALE:
 * - Content-Defined Chunking ensures that if bytes are inserted or deleted at the beginning of a file,
 *   subsequent chunk boundaries shift dynamically back into alignment. This optimizes deduplication and
 *   binary diffing efficiency for large files.
 * - Utilizes a deterministic Gear-based rolling hash table to search for boundaries.
 * - Restricts chunk sizes between 8KB and 64KB (averaging ~16KB) to balance indexing overhead
 *   against deduplication resolution.
 */
